...
for(i in 1:9){
  ...
  for(j in 1:10){
    ...
    counter <- counter+1
  #train <- sample(nrow(AutoDataPoly), nrow(AutoDataPoly)/2)
  AutoDataPolyLM <- glm(mpg~poly(horsepower,j, raw=TRUE), data = AutoDataPoly)
...
for(i in 1:10){
  AutoDataPolyLM <- glm(mpg~poly(horsepower,i, raw=TRUE) + poly(cylinders, i, raw=TRUE) + poly(displacement, i, raw=TRUE) + poly(weight, i, raw=TRUE) + poly(acceleration,i, raw=TRUE) + poly(year,i, raw=TRUE) + poly(origin, i, raw=TRUE), data = AutoDataPoly)
...
The variables "origin" and"cylindars" could not be used with an orthogonal polynomial of the 10th order. However, normal polynomials (used here) result in higher confidence intervals, and so reduced signifigance of coeficients. Our output shows the significance of higher orders of horsepower year, and possibly origin (but there are too few levels of this) to improve with higher order polynomials, suggesting highly curvelinier data. However, significance reduces with the other variables, suggesting the opposite.
